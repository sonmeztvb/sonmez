...
exports.run = async (client, message, args) => {
	if(!args[0]) {
		const embed = new Discord.RichEmbed()
			.setDescription(` **Lütfen Sayacı Ayarlamam İçin Bir Sayı Belirt! **örnek: +sayaç 100**`)
	...
	if(isNaN(args[0])) {
		const embed = new Discord.RichEmbed()
			.setDescription(`Lütfen Sayacı Ayarlamam İçin Bir Sayı Belirt **örnek: n!sayaç 5000**`)
			.setColor(ayarlar.renk)
		message.channel.send({embed})
		return
	}
